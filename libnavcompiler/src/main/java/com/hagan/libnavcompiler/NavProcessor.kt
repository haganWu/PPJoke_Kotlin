package com.hagan.libnavcompiler
...
@SupportedAnnotationTypes(
    "com.hagan.libnavannotation.FragmentDestination",
    "com.hagan.libnavannotation.ActivityDestination"
)
class NavProcessor : AbstractProcessor() {
    ...
    override fun process(
        ...
        if (fragmentDestination.isNotEmpty() || activityDestination.isNotEmpty()) {
            ...
            //CLASS_PATH 和 StandardLocation.SOURCE_PATH用的不多，指定了这个参数，就要指定生成文件的pkg包名了
            val resource =
            ...
            File(assetsPath).let { file ->
                ...
                File(file, outputFileName).let { newFile ->
                    if (newFile.exists()) {
                        newFile.delete()
                    }
    ...
    private fun handleDestination(
        ...
        elements.forEach {
            ...
            //获取具体的注解
            val annotation = it.getAnnotation(clazz)
